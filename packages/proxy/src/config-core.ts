...
function buildTlsContext(host: string, trustedCaPath: string) {
  return {
...
function buildCluster(
  ...
  return {
    ...(endpoint.protocol === "https"
      ? {
          transport_socket: buildTlsContext(endpoint.host, trustedCaPath),
        }
      : {}),
    connect_timeout: "5s",
...
function buildProviderClusters(providers: NormalizedProxyProvider[], trustedCaPath: string) {
  const seen = new Set<string>();
  ...
  for (const provider of providers) {
    ...
    clusters.push(buildCluster(clusterName, endpoint, trustedCaPath));
  }
    ...
    static_resources: {
      clusters: [
        buildCluster(
          ...
          options.trustedCaPath,
        ),
        ...buildProviderClusters(options.providers, options.trustedCaPath),
      ],
      listeners: [
        {
          address: {
...
export function generateGatewayConfigCore(options: ProxyGatewayOptions, trustedCaPath: string) {
